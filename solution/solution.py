...
def get_calls(elevator_id, calls):
    # 애초에 출발, 도착 일때, 짝짝, 홀짝, 짝홀, 홀홀 로 나누어 관리했지만 홀홀이 많은 케이스로 인해 무규칙 배분이 더 효과적임
    # calls = filter(lambda call: (int(call['start']) % 2) == (elevator_id // 2) and (int(call['end']) % 2) ==(elevator_id % 2), calls)
    # calls = sorted(calls, key=lambda call: (call['start'], call['end']))
    print("*************elevator_id: %s* %s, %s *******" % (elevator_id,  elevator_id // 2, elevator_id % 2))
    print(calls)
    return calls
